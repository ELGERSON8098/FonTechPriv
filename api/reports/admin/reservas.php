...
if (isset($_GET['idReserva'])) {
    ...
    if ($reserva->setIdReserva($_GET['idReserva']) && $pedido->setIdDetalle($_GET['idReserva'])) {
        ...
        if ($rowReserva = $reserva->readOne()) {
            ...
            $pdf->startReport('Producto reservado del cliente ' . $rowReserva['nombre_usuario']);
            ...
            if ($dataPedido = $pedido->readDetail()) {
                ...
                $pdf->cell(30, 10, 'Estado reserva', 1, 1, 'C', 1);
                // Se establece la fuente para los datos de los productos.
                $pdf->setFont('Arial', '', 11);
                // Se recorren los registros fila por fila.
                foreach ($dataPedido as $rowDetalle) {
                    ($rowDetalle['estado_reserva']) ? $estado = 'Aceptado' : $estado = 'Pendiente';
                    // Se imprimen las celdas con los datos de los productos.
                    
                    $pdf->cell(50, 10, $pdf->encodeString($rowDetalle['nombre_producto']), 1, 0);
                    $pdf->cell(50, 10, $rowDetalle['cantidad'], 1, 0);
                    $pdf->cell(30, 10, $rowDetalle['precio_unitario'], 1, 0);
